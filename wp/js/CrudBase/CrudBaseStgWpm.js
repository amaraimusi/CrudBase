/**
 * CrudBaseストラテジー：WordPress管理画面用（ダッシュボード内の画面）
 * @version 1.0.0
 * @date 2020-3-30
 * @license MIT
 * @auther kenji uehara
 */
class CrudBaseStgWpm{
	...
	setParamIfEmpty(param){
		...
		let wp_ajax_url = param.wp_ajax_url; // WordPress管理画面用、AjaxURL
		console.log('wp_ajax_url＝' + wp_ajax_url);//■■■□□□■■■□□□)
		...
		if(param['edit_reg_url'] == null){
			param['edit_reg_url'] = wp_ajax_url;
		}

		// 新規登録サーバーURL
		if(param['new_reg_url'] == null){
			param['new_reg_url'] = wp_ajax_url;
		}

		// 削除登録サーバーURL
		if(param['delete_reg_url'] == null){
			param['delete_reg_url'] = wp_ajax_url;
		}

		// 自動保存サーバーURL
		if(param['auto_save_url'] == null){
			param['auto_save_url'] =  wp_ajax_url;
		}
...
//		}
	 ...
	 * @return FormData
	 */
	editRegBeforeHook(fd){
		var action = this.param.wp_ajax_action;
		fd.append('action', action);
		fd.append('nonce', this.param.nonce);
		fd.append('scr_key', this.param.scr_key);
		fd.append('method_name', 'ajax_reg');
