...
class CrudBase{
	 ...
	 * 
	 * @param param
	 * - fw_strategy_typ 'cake':Cakeストラテジー（デフォルト）, 'wpm':WordPress管理画面ストラテジー
	 */
	constructor(param,fieldData){

		// --- 初期化: CrudBaseBaseクラスのメンバを初期化する ----

		this.strategy = null; // フレームワーク・ストラテジー
	...
	_setParamIfEmpty(param){
		...
		let param_json = jQuery('#param_json').val();
		let param2 = JSON.parse(param_json);
		$.extend(param, param2); // マージ
		
		if(param.fw_strategy_type == 'wpm'){
			this.strategy = new CrudBaseStgWpm();
		}else{
			this.strategy = new CrudBaseStgCake();
		}

		param = this.strategy.setParamIfEmpty(param);
		return param;
	}
	...
	_factoryClmShowHide(){
		...
		var csh_json = this.param.csh_json;
...
//		console.log('this.param.edit_reg_url＝' );//■■■□□□■■■□□□)
//		console.log(this.param.edit_reg_url);//■■■□□□■■■□□□)
//		console.log(this.param);//■■■□□□■■■□□□)
		...
		// フレームワークストラテジーごとに異なる処理
		if(this.strategy.editRegBeforeHook){
			fd = this.strategy.editRegBeforeHook(fd);
		}
