 ...
 * 
 * @version 3.0.0
 * @date 2016-1-21 | 2020-3-26
 * @history
 * 2020-3-26 v3.0.0 Cakeからの脱却
...
class CrudBaseModel{
	...
	public function getData(&$param, &$where){
		...
		// テーブル名を取得する
		$model_name_s = $param['model_name_s'];
		$tbl_name = $model_name_s . 's';
		
		// テーブル名のエイリアス
		$tbl_name_a = $param['model_name_c'];
		
		$pages = $param['pages'];//ページネーション情報
		
		// ORDER句の組み立て
		$order = '';
		if(!empty($pages['sort_field'])){
			$order = "ORDER BY {$tbl_name_a}.{$pages['sort_field']}";
			if(!empty($pages['sort_desc'])){
				$order .= ' DESC';
			}
		}
		...
		// ページ番号取得
		$page_no = 0;
		if(!empty($pages['page_no'])) $page_no = $pages['page_no'];
		
		// 行制限数を取得
		$row_limit = 0;
		if(!empty($pages['row_limit'])) $row_limit = $pages['row_limit'];
		...
		// LIMITがかかっていないデータ件数を取得する
		$found_row_count = $this->dao->getValue("SELECT FOUND_ROWS();");
		// ■■■□□□■■■□□□委
		//$found_row_count = $wpdb->get_var("SELECT FOUND_ROWS();");
